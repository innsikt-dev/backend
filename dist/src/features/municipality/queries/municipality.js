...
export async function queryMunicipality(id) {
    ...
    ON 
        m.id = i.municipality_id
    WHERE LOWER(municipality_name) = LOWER($1);`, [id]);
    return {
        municipality: municipality.rows.length === 0 ? [] : municipality.rows[0],
        municipalityIncidents: municipalityIncidents.rows.length === 0
            ? []
            : municipalityIncidents.rows[0].amount,
        municipalityCommonCategory: municipalityCommonCategory.rows.length === 0
            ? []
            : municipalityCommonCategory.rows[0].category,
        daysWithData: daysWithData.rows.length === 0 ? [] : daysWithData.rows[0].amount,
    };
}
