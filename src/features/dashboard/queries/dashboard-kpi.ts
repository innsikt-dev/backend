...
export async function queryDashboardKpi(): Promise<DashboardKpi> {
  ...
  const mostCommonCategory = await db.query(
    `SELECT type AS category, COUNT(type)
     FROM mv_dashboard_base
     WHERE created_on::date = CURRENT_DATE
     GROUP BY category
     ORDER BY COUNT(*) DESC
